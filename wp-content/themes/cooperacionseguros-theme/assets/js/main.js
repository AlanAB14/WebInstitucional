...
(function (factory) {
  ...
  $.validator.addMethod(
    ...
    $.validator.format(
      "Por favor ingresá un archivo con los formatos aceptados."
    )
  );
  ...
  // Limit the size of each individual file in a FileList (in kb)
  $.validator.addMethod(
    "maxsize",
    function (value, element, param) {
      if (this.optional(element)) {
      ...
      if ($(element).attr("type") === "file") {
        if (element.files && element.files.length) {
          for (var i = 0; i < element.files.length; i++) {
            //console.log(element.files[i].size / 1000);
            if (element.files[i].size / 1000 > param) {
              return false;
            }
          }
        }
    ...
    $.validator.format("Los archivos no pueden exceder los {0}kb.")
  );
...
$(function () {
  ...
  $.templates({
    ...
    templatePreguntas:
      ...
      {{else question === "location"}} {{include tmpl="templatePreguntasUbicacion"/}}\
      {{else question === "vehicleBrand"}} {{include tmpl="templatePreguntasMarca"/}}\
      {{else question === "vehicleModel"}} {{include tmpl="templatePreguntasModelo"/}}\
      {{else question === "vehicleYear"}} {{include tmpl="templatePreguntasAno"/}}\
      {{else question === "vehicleVersion"}} {{include tmpl="templatePreguntasVersion"/}}\
      {{else question === "vehicleGnc"}} {{include tmpl="templatePreguntasGnc"/}}\
  ...
  $(document).on("click", "form .productbutton", function () {
    ...
    if (
      ...
      // Send tracking of category and product selection
      trackSelector(currentProduct.slug);
    } else { // Si no tiene subproductos, chequear si tiene preguntas
      if (
      ...
      // Send tracking of category and product selection
      trackSelector(currentProduct.category, currentProduct.slug);
    }
  });
  ...
  function showcurrentQuestion(i, totalQuestions) {
    ...
    if (
      questions[i] == "location" ||
      questions[i] == "vehicleBrand" ||
      questions[i] == "vehicleModel" ||
      questions[i] == "vehicleYear" ||
      questions[i] == "vehicleVersion" ||
      questions[i] == "vehicleGnc" ||
      ...
      } else if (questions[i] == "vehicleVersion") {
        if (category == 2) {
          var dataFile =
      ...
      } else if (questions[i] == "vehicleYear") {
        var dataFile = yearsFile + "&model=" + userData.vehicleModelId;
        var dataField = "";
      } else if (questions[i] == "vehicleGnc") {
  ...
  // Enviar gtag track al seleccionar categoría o producto. (Selector)
  function trackSelector(category, product = null, origin = 'home') {
    clearDataLayer();
  ...
  // Enviar gtag track en cada pregunta. (Preguntador)
  function trackQuestioner() {
    const question = cachedQuestions[currentQuestion];
    const trackeableAnswers = ['vehicleBrand', 'vehicleModel', 'vehicleYear', 'vehicleVersion', 'vehicleGnc', 'location'];
    const answer = (question == 'location') ? `${userData['userCity']}, ${userData['userState']}, ${userData['userZip']}` : userData[question];

    const event = {
      'event': 'trackQuestioner',
      'product': userData.currentProduct,
      'question': question,
    };

    if (trackeableAnswers.includes(question)) {
  ...
  $(document).on("click", ".nextquestion", function (event) {
    ...
    let isvalid = productContainer.valid();
  ...
  $(document).on("click", ".closeproducts", function () {
    ...
    // Borrar todo el contenido si cierra en una landing
    if (isLanding) {
      $("#productos").remove();
    }
  ...
  $(document).on("click", ".productLauncher", function (event) {
    ...
    $.getJSON(jsonFile, function (data) {
      ...
      // Send tracking of category and product selection
      trackSelector(currentProduct.category, currentProduct.slug, 'landing');
    });
  });
...
$(function () {
  ...
  if (planesContainer.length) {
    ...
    function trackEcommerceImpression(plan) {
      if (impressedPlans.includes(plan)) return;

      const eventData = {
        'event': 'trackEcommerceImpression',
        ...commonEvent,
      };

      if (product == 'seguro-de-motos' || product == 'seguro-de-autos-y-pick-ups') {
        const { cobertura, premioMensual } = plans[plan];
        eventData['vehiclePlan'] = cobertura;
        eventData['vehiclePlanPrice'] = premioMensual + (ap.premioMensual ?? 0);
        eventData['vehiclePlanPriceAP'] = ap.premioMensual ?? 0;
        eventData['vehiclePlanPriceSubtotal'] = premioMensual;
      } else if (product == 'seguro-de-vida-individual') {
        eventData['lifeInsurancePlan'] = plan;

        if (plan == 'esencial') {
          eventData['lifeInsurancePlanPrice'] = Math.ceil(plans[0].premio / 12);
        } else if (plan == 'plus') {
          eventData['lifeInsurancePlanPrice'] = Math.ceil(plans[1].premio / 12);
        }
      }

      pushDataLayer(eventData);

      impressedPlans.push(plan);
    }

    if (product == 'seguro-de-motos' || product == 'seguro-de-autos-y-pick-ups') {
      if (recommended['basic']) trackEcommerceImpression(recommended['basic']);
      if (recommended['medium']) trackEcommerceImpression(recommended['medium']);
      if (recommended['full']) trackEcommerceImpression(recommended['full']);
    } else if (product == 'seguro-de-vida-individual') {
      trackEcommerceImpression('esencial');
      trackEcommerceImpression('plus');
    }
    ...
    $(".enviar").on("click", function (event) {
      ...
      if (product == "seguro-de-autos-y-pick-ups" || product == 'seguro-de-motos') {
        planesContainer
      ...
      // Enviar trackInitCheckout
      trackInitCheckout(selectedPlan);
...
$(function () {
  ...
  $("form#vehicle-details").validate({
    submitHandler: (form, event) => {
      const eventData = {
        'event': 'trackEcommerceCheckoutVehicleDetails',
        ...commonEvent,
      };
  ...
  $("form#vehicle-photos button").on("click", function (event) {
    ...
    const eventData = {
      'event': 'trackEcommerceCheckoutVehiclePhotos',
      ...commonEvent,
    };
  ...
  $("#imagen-adicional").on("click", function (event) {
    event.preventDefault();
  ...
  $("#customerLocalidadActual").on("select2:select", function (event) {
  ...
  $("#datos-solicitante").validate({
    ...
    submitHandler: function (form, event) {
      const eventData = {
        'location': $('select[name="customerLocalidadActual"] option:selected').text(),
        ...commonEvent,
      };

      if (purchasable) {
        eventData['event'] = 'trackEcommerceCheckoutCustomerDetails';
      } else {
        eventData['event'] = 'trackCheckoutCustomerDetails';
      }

      if (event.target.elements.num_dni.value) {
        eventData['customerCitizenship'] = $('select[name="customerNacionalidad"] option:selected').text();
        eventData['customerOccupation'] = $('select[name="customerOcupacion"] option:selected').text();
        eventData['customerCivilStatus'] = $('select[name="customerEstadoCivil"] option:selected').text();
      }
  ...
  $("form#advisors").validate({
    submitHandler: function (form, event) {
      const advisorID = $('input[name="cod_productor"]:checked').val();
      const [advisor] = advisors.filter(({ codigoProductor }) => codigoProductor == advisorID);
      const eventData = {
        'advisor': advisor.productor,
        ...commonEvent,
      };

      if (purchasable) {
        eventData['event'] = 'trackEcommerceCheckoutAdvisor';
      } else {
        eventData['event'] = 'trackCheckoutAdvisor';
      }
  ...
  $("form#vehicle-summary").validate({
    submitHandler: function (form, event) {
      const eventData = {
        'event': 'trackEcommerceCheckoutStartPayment',
        ...commonEvent,
      };
...
function saveStep(id, data) {
  ...
  $.post(themePath + "inc/ajax-terceros.php", {
    ...
    if (!data) {
      console.error('Error when submiting data: ', parsedData);
    }
  });
}
    ...
    */
    ...
    $(".continuar button").click(function (event) {
      ...
      if (formContainer.valid() && checkboxes) {
        saveStep(id, formContainer.serializeArray());
        ...
        if (id == "paso6") {
          ...
          formContainer.submit(procesarDatos(event));
        } else {
          // Ver próximo paso
          nextStep($(this));
        }
