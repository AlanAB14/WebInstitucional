  ...
  public function __construct() {
    ...
    if (isset($this->quote['answers']['planFechaFin'])) {
      if ((strtotime("now") > strtotime($this->quote['answers']['planFechaFin'])) && ($this->step->slug !== $this->steps[0]->slug)) {
        header('Location: /' . $this->product->slug . '/' . $this->guid . '/?estado=vencido');
        exit;
      }
    }
    ...
    $lead = new Lead($this->quote, $this->step->id, $this->step->title);
    $lead->send_lead();
  }
  ...
  public function parse_post() {
    ...
    switch($slug) {
      ...
      case 'resumen':
        ...
        if ($idPropuesta >= 1) {
          $this->quote['idPropuesta'] = $idPropuesta;
          ...
          if ($idPoliza >= 1) {
            ...
            if ($data) {
              $lead = new Lead($this->quote, $this->steps[$this->step->id]->id, $this->steps[$this->step->id]->title);
              $lead->send_lead();
      ...
      case 'pago':
        ...
        $this->quote['pago']['status'] = $this->status;
