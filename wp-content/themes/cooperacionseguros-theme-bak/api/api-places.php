...
function coopseg_places_save_places($token)
{
  ...
  foreach ($result as $r) {
    ...
    $query = "INSERT INTO " . COOPSEG_PLACES_DB_TABLE . " (idcity,city,idstate,state,zipcode,search,quote,emit) VALUES (
      '" . $r->idLocalidad . "',
      '" . ucwords(strtolower($r->localidad)) . "',
      '" . $r->idProvincia . "',
      '" . ucwords(strtolower($r->provincia)) . "',
      '" . $r->codigoPostal . "',
      '" . ucwords(strtolower($r->localidad)) . " " . ucwords(strtolower($r->provincia)) . " " . $r->codigoPostal . "',
      '" . $cotizaEnLocalidad . "',
      '" . $emiteEnLocalidad . "'
    )";
  ...
  foreach ($resultProductores as $r) {
    ...
    `altName` = '" . ucwords(strtolower($r->localidad)) . "',
    `producers` = 1
